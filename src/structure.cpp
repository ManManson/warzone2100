...
# pragma GCC diagnostic pop
		...
		if (!setFunctionality(psBuilding, pStructureType->type))
		{
			...
			auto& globalStructStorage = GlobalStructContainer();
			auto it = globalStructStorage.find(*psBuilding);
			ASSERT(it != globalStructStorage.end(), "buildStructureDir(): psBuilding not found in global storage");
			GlobalStructContainer().erase(it);
