		...
		if ((intMode == INT_OBJECT || intMode == INT_STAT) && objMode == IOBJ_BUILDSEL)
		{
			...
			else if (found3DBuilding(pos))	//found building
			{
					...
					!psSelectedBuilder->died)
				{
					...
					{
						if (fireOnLocation(activeGameWorld(), pos.x, pos.y))
						{
			...
			if (pos.x != INT32_MAX)
			{
				...
				for (int i = 0; i < lb.count; ++i)
				{
					...
					if (psBuilding && selectedPlayer < MAX_PLAYERS)
					{
						...
						{
							MAPTILE *psTile = mapTile(activeGameWorld(), map_coord(pos.x), map_coord(pos.y));
						...
						else
						{
							...
							tmp.pos = {pos.x, pos.y, map_Height(activeGameWorld(), pos.x, pos.y) + world_coord(1) / 10};
