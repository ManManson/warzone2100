...
void rayCast(GameWorld& world, Vector2i src, Vector2i dst, RAY_CALLBACK callback, void *data)
{
	if (!callback(world, src, 0, data) || src == dst)  // Start at src.
	{
	...
	while (canX || canY)
	{
		...
		if (!first)
		{
			...
			if (!worldOnMap(world, avg) || !callback(world, avg, iHypot(avg), data))
			{
	...
	if (!worldOnMap(world, dst))
	{
		return;  // Stop, since reached the edge of the map.
	}
	callback(world, dst, iHypot(dst), data);
}

//-----------------------------------------------------------------------------------
/* Will return false when we've hit the edge of the grid */
static bool getTileHeightCallback(GameWorld& world, Vector2i pos, int32_t dist, void *data)
{
	...
	{
		bool HasTallStructure = blockTile(world, map_coord(pos.x), map_coord(pos.y), AUX_MAP) & AIR_BLOCKED;
		...
		if (dist > TILE_UNITS || HasTallStructure)
		{
			...
			int height = map_Height(world, pos.x, pos.y), heightDiff;
			uint16_t newPitch;
...
void getBestPitchToEdgeOfGrid(GameWorld& world, UDWORD x, UDWORD y, uint16_t direction, uint16_t *pitch)
{
	HeightCallbackHelp_t help = {map_Height(world, x, y), 0};

	Vector3i src(x, y, 0);
	Vector3i delta(iSinCosR(direction, 5430), 0);
	rayCast(world, src.xy(), (src + delta).xy(), getTileHeightCallback, &help); // FIXME Magic value
