...
void rayCast(Vector2i src, Vector2i dst, RAY_CALLBACK callback, void *data)
{
	...
	while (canX || canY)
	{
		...
		if (!first)
		{
			...
			if (!worldOnMap(activeGameWorld(), avg) || !callback(avg, iHypot(avg), data))
			{
	...
	if (!worldOnMap(activeGameWorld(), dst))
	{
...
static bool getTileHeightCallback(Vector2i pos, int32_t dist, void *data)
{
	...
	{
		bool HasTallStructure = blockTile(activeGameWorld(), map_coord(pos.x), map_coord(pos.y), AUX_MAP) & AIR_BLOCKED;
		...
		if (dist > TILE_UNITS || HasTallStructure)
		{
			...
			int height = map_Height(activeGameWorld(), pos.x, pos.y), heightDiff;
			uint16_t newPitch;
...
{
	HeightCallbackHelp_t help = {map_Height(activeGameWorld(), x, y), 0};
