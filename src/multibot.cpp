...
bool recvDroid(NETQUEUE queue)
{
	...
	if ((pos.x == 0 && pos.y == 0) || pos.x > world_coord(activeGameWorld().map.width) || pos.y > world_coord(activeGameWorld().map.height))
	{
	...
	psDroid = reallyBuildDroid(activeGameWorld(), pT, pos, player, false, rot, id);
