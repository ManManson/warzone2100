...
static bool updateGraviton(EFFECT *psEffect, LightingData& lightData)
{
	...
	if (map_coord(static_cast<int32_t>(psEffect->position.x)) >= activeGameWorld().map.width || map_coord(static_cast<int32_t>(psEffect->position.z)) >= activeGameWorld().map.height)
	{
		return false;
	}

	int groundHeight = map_Height(activeGameWorld(), static_cast<int>(psEffect->position.x), static_cast<int>(psEffect->position.z));
	...
	{
		psTile = mapTile(activeGameWorld(), map_coord(static_cast<int32_t>(psEffect->position.x)), map_coord(static_cast<int32_t>(psEffect->position.z)));
...
static bool updateConstruction(EFFECT *psEffect)
{
	...
	if (TEST_CYCLIC(psEffect))
	{
		...
		if (static_cast<int>(psEffect->position.y) <= map_Height(activeGameWorld(), static_cast<int>(psEffect->position.x), static_cast<int>(psEffect->position.z)))
		{
			return false;
		}
...
bool readFXData(const char *fileName)
{
	...
	for (int i = 0; i < list.size(); ++i)
	{
		...
		if (EFFECT_FIRE == curEffect.group)
		{
			...
			if (timeLeftToRun > 0)
			{
				tileSetFire(activeGameWorld(), static_cast<int32_t>(curEffect.position.x), static_cast<int32_t>(curEffect.position.z), static_cast<uint32_t>(timeLeftToRun));
			}
		}
