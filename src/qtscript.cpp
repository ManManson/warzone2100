...
wzapi::scripting_instance* scripting_engine::loadPlayerScript(const WzString& path, int player, AIDifficulty difficulty)
{
	...
	globalVars["mapWidth"] = activeGameWorld().map.width;
	//== * ```mapHeight``` Height of map in tiles.
	globalVars["mapHeight"] = activeGameWorld().map.height;
...
bool triggerEvent(SCRIPT_TRIGGER_TYPE trigger, BASE_OBJECT *psObj)
{
	...
	for (auto *instance : scripts)
	{
		...
		case TRIGGER_START_LEVEL:
			processVisibility(activeGameWorld()); // make sure we initialize visibility first
			instance->handle_eventStartLevel();
			break;
...
void clearMarks()
{
	...
	{
		for (int x = 0; x < activeGameWorld().map.width; x++) // clear old marks
		{
...
void scripting_engine::showLabel(const std::string &key, bool clear_old, bool jump_to)
{
	...
	if (l.type == SCRIPT_AREA || l.type == SCRIPT_POSITION)
	{
		...
		{
			maxx = MIN(activeGameWorld().map.width, maxx + 1);
			maxy = MIN(activeGameWorld().map.height, maxy + 1);
		}
	...
	else if (l.type == SCRIPT_RADIUS)
	{
		...
		for (int x = MAX(map_coord(l.p1.x - l.p2.x), 0); x < MIN(map_coord(l.p1.x + l.p2.x), activeGameWorld().map.width); x++)
		{
			for (int y = MAX(map_coord(l.p1.y - l.p2.x), 0); y < MIN(map_coord(l.p1.y + l.p2.x), activeGameWorld().map.height); y++) // l.p2.x is radius, not a bug
			{
	...
	else if (l.type == OBJ_DROID || l.type == OBJ_FEATURE || l.type == OBJ_STRUCTURE)
	{
		...
		if (psObj)
		{
			...
			MAPTILE *psTile = mapTile(activeGameWorld(), map_coord(psObj->pos.x), map_coord(psObj->pos.y));
			psTile->tileInfoBits |= BITS_MARKED;
		}
	}
...
generic_script_object scripting_engine::getObject(WZAPI_PARAMS(wzapi::object_request request))
{
	if (request.requestType == wzapi::object_request::RequestType::MAPPOS_REQUEST)
	{
		...
		int x = pos.x;
		int y = pos.y;
		SCRIPT_ASSERT({}, context, tileOnMap(activeGameWorld(), x, y), "Map position (%d, %d) not on the map!", x, y);
...
wzapi::no_return_value scripting_engine::hackMarkTiles_ByLabel(WZAPI_PARAMS(const std::string& label))
{
	...
	else if (l.type == SCRIPT_RADIUS)
	{
		for (int x = map_coord(l.p1.x - l.p2.x); x < map_coord(l.p1.x + l.p2.x); x++)
		{
			...
			{
				if (x >= -1 && x < activeGameWorld().map.width + 1 && y >= -1 && y < activeGameWorld().map.width + 1 && iHypot(map_coord(l.p1) - Vector2i(x, y)) < map_coord(l.p2.x))
				{
					MAPTILE *psTile = mapTile(activeGameWorld(), x, y);
					psTile->tileInfoBits |= BITS_MARKED;
				}
