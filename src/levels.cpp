...
static bool levPrepareLoadEnvironment(LEVEL_DATASET* psNewLevel, const std::string& savePath)
{
	...
	if (psBaseData == nullptr && !savePath.empty())
	{
		if (!saveGameReset())
		{
...
static LevDatasetResolveResult levResolveDatasetForLoad(LevLoadContext& ctx)
{
	...
	ctx.bCamChangeSaveGame = !ctx.pSaveName.empty() && ctx.saveType == GTYPE_SAVE_START && ctx.psNewLevel->psChange != nullptr;
...
static bool levLoadMissionBranchesBeforeMainLoop(LevLoadContext& ctx)
{
	...
	if (psNewLevel->game == -1)  //no .gam file to load - BETWEEN missions (for Editor games only)
	{
		...
		if (!ctx.pSaveName.empty())
		{
			...
			debug(LOG_NEVER, "Loading savegame: %s", ctx.pSaveName.c_str());
			if (!loadGame(GameLoadDetails::makeUserSaveGameLoad(ctx.pSaveName), false, true))
			{
				debug(LOG_ERROR, "Failed loadGame(%s)!", ctx.pSaveName.c_str());
				return false;
			}
		}

		if (ctx.pSaveName.empty() || ctx.saveType == GTYPE_SAVE_START)
		{
...
static bool levLoadMissionDataLoop(LevLoadContext& ctx)
{
	...
	for (int i = 0; i < LEVEL_MAXFILES; i++)
	{
		if (psNewLevel->game == i)
		{
			...
			if (!ctx.pSaveName.empty() && !ctx.bCamChangeSaveGame)
			{
				//set the mission type before the saveGame data is loaded
				if (ctx.saveType == GTYPE_SAVE_MIDMISSION)
				{
				...
				debug(LOG_NEVER, "Loading save game %s", ctx.pSaveName.c_str());
				if (!loadGame(GameLoadDetails::makeUserSaveGameLoad(ctx.pSaveName), false, true))
				{
					debug(LOG_ERROR, "Failed loadGame(%s)!", ctx.pSaveName.c_str());
					return false;
...
static bool levFinalizeLevelLoad(LevLoadContext& ctx)
{
	...
	if (!ctx.pSaveName.empty())
	{
		...
		if (!loadMissionExtras(ctx.pSaveName.c_str(), psNewLevel->type))
		{
			debug(LOG_ERROR, "Failed loadMissionExtras(%s, %d)!", ctx.pSaveName.c_str(), static_cast<int8_t>(psNewLevel->type));
			return false;
		}
	}

	if (!ctx.pSaveName.empty() && ctx.saveType == GTYPE_SAVE_MIDMISSION)
	{
		//load script stuff
		// load the event system state here for a save game
		debug(LOG_SAVE, "Loading script system state");
		if (!loadScriptState(const_cast<char *>(ctx.pSaveName.c_str())))
		{
			debug(LOG_ERROR, "Failed loadScriptState(%s)!", ctx.pSaveName.c_str());
			return false;
		}
	}
...
bool levLoadData(char const *name, Sha256 const *hash, char *pSaveName, GAME_TYPE saveType)
{
	...
	LevLoadContext ctx;
	ctx.name = name != nullptr ? name : "";
	ctx.hash = hash;
	ctx.pSaveName = pSaveName != nullptr ? pSaveName : "";
