...
bool cmdDroidAddDroid(DROID *psCommander, DROID *psDroid)
{
	...
	if (psCommander->psGroup->getNumMembers() < cmdDroidMaxGroup(psCommander))
	{
		...
		ASSERT(psCommander->owningWorld != nullptr && psCommander->owningWorld == psDroid->owningWorld, "Commander and unit must share a world");
		secondarySetState(psDroid, psCommander->owningWorld->objects, DSO_REPAIR_LEVEL, (SECONDARY_STATE)(psCommander->secondaryOrder & DSS_REPLEV_MASK), ModeImmediate);
		secondarySetState(psDroid, psCommander->owningWorld->objects, DSO_ATTACK_LEVEL, (SECONDARY_STATE)(psCommander->secondaryOrder & DSS_ALEV_MASK), ModeImmediate);
		secondarySetState(psDroid, psCommander->owningWorld->objects, DSO_HALTTYPE, (SECONDARY_STATE)(psCommander->secondaryOrder & DSS_HALT_MASK), ModeImmediate);
