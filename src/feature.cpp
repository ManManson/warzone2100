			...
			for (int width = 0; width < b.size.x; ++width)
			{
				...
				if (terrainType(psTile) != TER_WATER)
				{
					if (terrainType(psTile) != TER_CLIFFFACE)
					{
						...
						if (isUrban)
						{
							makeTileRubbleTexture(activeGameWorld(), psTile, x, y, RUBBLE_TILE);
						}
						auxClearBlocking(activeGameWorld(), x, y, AUXBITS_ALL);
					}
					else
					{
						/* This remains a blocking tile */
						psTile->psObject = nullptr;
						auxClearBlocking(activeGameWorld(), x, y, AIR_BLOCKED);  // Shouldn't remain blocking for air units, however.
						if (isUrban)
						{
							makeTileRubbleTexture(activeGameWorld(), psTile, x, y, BLOCKING_RUBBLE_TILE);
						}
					}
				}
			}
