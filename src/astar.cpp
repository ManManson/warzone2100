...
struct PathfindContext
{
	...
	bool isBlocked(int x, int y) const
	{
		...
		return x < 0 || y < 0 || x >= activeGameWorld().map.width || y >= activeGameWorld().map.height || blockingMap->map[x + y * activeGameWorld().map.width];
	}
	bool isDangerous(int x, int y) const
	{
		return !blockingMap->dangerMap.empty() && blockingMap->dangerMap[x + y * activeGameWorld().map.width];
	}
	...
	void assign(const std::shared_ptr<const PathBlockingMap> &blockingMap_, PathCoord tileS_, PathNonblockingArea dstIgnore_)
	{
		...
		map.resize(static_cast<size_t>(activeGameWorld().map.width) * static_cast<size_t>(activeGameWorld().map.height));  // Allocate space for map, if needed.
	}
...
{
	ASSERT_OR_RETURN(, (unsigned)pos.x < (unsigned)activeGameWorld().map.width && (unsigned)pos.y < (unsigned)activeGameWorld().map.height, "X (%d) or Y (%d) coordinate for path finding node is out of range!", pos.x, pos.y);
	...
	PathExploredTile &expl = context.map[pos.x + pos.y * activeGameWorld().map.width];
...
static PathCoord fpathAStarExplore(PathfindContext &context, PathCoord tileF)
{
	...
	while (!context.nodes.empty() && !foundIt)
	{
		...
		if (context.map[node.p.x + node.p.y * activeGameWorld().map.width].visited)
		{
			continue;  // Already been here.
		}
		context.map[node.p.x + node.p.y * activeGameWorld().map.width].visited = true;
...
ASR_RETVAL fpathAStarRoute(const std::shared_ptr<FPathExecuteContext>& ctx, MOVE_CONTROL *psMove, PATHJOB *psJob)
{
	...
	for (; contextIterator != fpathContexts.end(); ++contextIterator)
	{
		...
		if (contextIterator->map[tileOrig.x + tileOrig.y * activeGameWorld().map.width].iteration == contextIterator->iteration
		    && contextIterator->map[tileOrig.x + tileOrig.y * activeGameWorld().map.width].visited)
		{
	...
	{
		ASSERT_OR_RETURN(ASR_FAILED, worldOnMap(activeGameWorld(), p.x, p.y), "Assigned XY coordinates (%d, %d) not on map!", (int)p.x, (int)p.y);
		ASSERT_OR_RETURN(ASR_FAILED, path.size() < (static_cast<size_t>(activeGameWorld().map.width) * static_cast<size_t>(activeGameWorld().map.height)), "Pathfinding got in a loop.");

		path.push_back(p);

		PathExploredTile &tile = context.map[map_coord(p.x) + map_coord(p.y) * activeGameWorld().map.width];
...
void fpathSetBlockingMap(PATHJOB *psJob)
{
	...
	if (i == fpathBlockingMaps.end())
	{
		...
		map.resize(static_cast<size_t>(activeGameWorld().map.width) * static_cast<size_t>(activeGameWorld().map.height));
		...
		for (int y = 0; y < activeGameWorld().map.height; ++y)
			...
			{
				map[x + y * activeGameWorld().map.width] = fpathBaseBlockingTile(activeGameWorld(), x, y, type.propulsion, type.owner, type.moveType);
				checksumMap ^= map[x + y * activeGameWorld().map.width] * (factor = 3 * factor + 1);
			}
		if (!isHumanPlayer(type.owner) && type.moveType == FMT_MOVE)
		{
			std::vector<bool> &dangerMap = blockMap->dangerMap;
			dangerMap.resize(static_cast<size_t>(activeGameWorld().map.width) * static_cast<size_t>(activeGameWorld().map.height));
			for (int y = 0; y < activeGameWorld().map.height; ++y)
				for (int x = 0; x < activeGameWorld().map.width; ++x)
				{
					dangerMap[x + y * activeGameWorld().map.width] = auxTile(activeGameWorld(), x, y, type.owner) & AUXBITS_THREAT;
					checksumDangerMap ^= dangerMap[x + y * activeGameWorld().map.width] * (factor = 3 * factor + 1);
				}
		}
